        ...
        <div className={styles.page}>
            ...
            <div className={`card ${styles.formCard}`}>
                ...
                <form onSubmit={handleSubmit} className={styles.form}>
                    ...
                    <div className={styles.dogPickerSection}>
                        <label className={`input-label ${styles.dogPickerLabel}`}>
                            🐕 Related Dogs
                            <span className={styles.dogPickerOptional}>(Optional - leave blank for general questions)</span>
                        </label>
                        {dogs.length > 0 ? (
                            <div className={styles.dogPickerGrid}>
                                {dogs.map((dog) => (
                                    <button
                                        key={dog.id}
                                        type="button"
                                        onClick={() => toggleDogSelection(dog.id)}
                                        className={`${styles.dogPickerOption} ${selectedDogIds.includes(dog.id) ? styles.selected : ''}`}
                                    >
                                        {dog.photo ? (
                                            <img src={dog.photo} alt={dog.name} className={styles.dogPickerPhoto} />
                                        ) : (
                                            <span className={styles.dogPickerPhoto}>🐕</span>
                                        )}
                                        <span className={styles.dogPickerName}>{dog.name}</span>
                                        <span className={styles.dogPickerBreed}>({dog.breed})</span>
                                        <span className={styles.dogPickerCheckmark}>✓</span>
                                    </button>
                                ))}
                            </div>
                        ) : (
                            <div className={styles.noDogs}>
                                <span>No dogs added yet.</span>
                                <Link href="/dogs/new">Add a dog profile</Link>
                            </div>
                        )}
                    </div>
